...
function readMultipleFiles(input) {
  if (input.files && input.files.length > 0) {
    ...
    $("#multipleFileUpload .image-title").html(
...
function removeMultipleUpload() {
  $("#multipleFileUpload .file-upload-input").replaceWith(
    $("#multipleFileUpload .file-upload-input").clone(),
  );
...
function uploadMultipleFiles() {
  ...
  if (fileInput.files.length > 0) {
    ...
    fetch(`${apiBaseUrl}/upload-multiple`, {
      // Gọi API uploadMultiple
      method: "POST",
      body: formData,
    })
      .then((response) => response.json())
...
const showChunkForm = (input) => {
  ...
  if (input.files && input.files.length > 0) {
    ...
    $("#largeFileUpload .image-title").html(
      `${input.files.length} files selected`,
    );
    $("#totalChunks").html(`Total chunks: ${totalChunks}`);
  }
...
function removeLargeFileUload() {
  $("#largeFileUpload .file-upload-input").replaceWith(
    $("#largeFileUpload .file-upload-input").clone(),
  );
...
const uploadResumable = async () => {
  ...
  const initResponse = await fetch(
    "http://localhost:3001/api/upload-large/init",
    {
      method: "POST",
    },
  );
  ...
  for (let i = startChunk; i < Math.min(totalChunks, endChunk); ++i) {
    ...
    try {
      const response = await fetch(
        "http://localhost:3001/api/upload-large/upload-chunk",
        {
          method: "POST",
          body: formData,
        },
